...
def run(message, bot):
    ...
    bot.register_next_step_handler(msg, users_email_input, bot)

def users_email_input(message, bot):
    try:
        chat_id = message.chat.id
        ...
        message = bot.send_message(chat_id, 'Please enter comma separated email ids of all the users you want to add in the expense. \n The first email in the response will be designated as the expense creator \n')
...
def take_all_users_input(message, bot, selected_category):
    try:
        ...
        if not validate_email_input(emails):
        ...
        for email in set(emails.split(",")):
            ...
            user_ids_present_in_expense.append(email)

        option[chat_id] = selected_category
        message = bot.send_message(chat_id, 'How much did you spend on {}? \n(Enter numeric values only)'.format(str(option[chat_id])))
        bot.register_next_step_handler(message, post_amount_input, bot, selected_category, user_ids_present_in_expense)
    except Exception as e:
        logging.exception(str(e))
        bot.reply_to(message, 'Oh no! ' + str(e))
        display_text = ""
...
def post_amount_input(message, bot, selected_category, user_ids_present_in_expense):
    try:
        chat_id = message.chat.id
...
def validate_email_input(emails):
