...
def test_visualize(mocker):
    mocker.patch.object(graph, 'plt')
    graph.plt.bar.return_value = True
    graph.visualize(dummy_total_text_data)
    graph.plt.bar.assert_called_with(dummy_categ_val.keys(), ANY, color=dummy_color, edgecolor=dummy_edgecolor)