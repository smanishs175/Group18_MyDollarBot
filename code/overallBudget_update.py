...
def run(message, bot):
    ...
    if utils.isOverallBudgetAvailable(chat_id):
        update_overall_budget(chat_id, bot)
    elif utils.isCategoryBudgetAvailable(chat_id):
        update_category_budget(message, bot)
    else:
        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
        options = utils.getBudgetTypes()
        markup.row_width = 2
...
def update_overall_budget(chat_id, bot):
    if (utils.isOverallBudgetAvailable(chat_id)):
        currentBudget = utils.getOverallBudget(chat_id)
...
def post_overall_amount_input(message, bot):
    try:
        chat_id = message.chat.id
        amount_value = utils.validate_entered_amount(message.text)
        if amount_value == 0:
...
def update_category_budget(message, bot):
    markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
    categories = utils.getSpendCategories()
    markup.row_width = 2
    for c in categories:
        markup.add(c)
...
def post_category_selection(message, bot):
    try:
        ...
        if utils.isCategoryBudgetByCategoryAvailable(chat_id, selected_category):
            currentBudget = utils.getCategoryBudgetByCategory(chat_id, selected_category)
...
def post_category_add(message, bot):
    markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
    options = utils.getUpdateOptions().values()
    markup.row_width = 2
    for c in options:
        markup.add(c)
...
def post_option_selection(message, bot):
    ...
    options = utils.getUpdateOptions()
    print("here")
